...
% disp(cfoCorrectedData)

% 
% 
% % Save CFO-corrected time-domain data without using frameNum
% outDir = 'cfoCorrected';
% if ~exist(outDir,'dir'), mkdir(outDir); end
% 
% % persistent counter for file names
% persistent cfoSaveCount;
% if isempty(cfoSaveCount)
%     cfoSaveCount = 0;
% end
% cfoSaveCount = cfoSaveCount + 1;
% 
% % build filename using our counter
% fname = fullfile(outDir, sprintf('%03d.mat', cfoSaveCount));
% data  = cfoCorrectedData;   %# rename so the .mat contains “data”
% save(fname, 'data');
% 
% 
...
% 
% disp(refSamples)
% outDir = 'refsamples'; % Directory for saving equalized symbols
% if ~exist(outDir, 'dir'), mkdir(outDir); end
% 
% % Persistent counter for file names
% persistent refSaveCount;
% if isempty(refSaveCount)
%     refSaveCount = 0;
% end
% refSaveCount = refSaveCount + 1;
% 
% % Build filename using the counter
% fname = fullfile(outDir, sprintf('%03d.mat', refSaveCount));
% data = refSamples; % Rename eqSym to 'data' for consistency in .mat file
% save(fname, 'data');
% 
...
% 
% outDir = 'demodulateddata'; % Directory for saving equalized symbols
% if ~exist(outDir, 'dir'), mkdir(outDir); end
% 
% % Persistent counter for file names
% persistent demoSaveCount;
% if isempty(demoSaveCount)
%     demoSaveCount = 0;
% end
% demoSaveCount = demoSaveCount + 1;
% 
% % Build filename using the counter
% fname = fullfile(outDir, sprintf('%03d.mat', demoSaveCount));
% data = demodulatedData; % Rename eqSym to 'data' for consistency in .mat file
% save(fname, 'data');
...
% 
% 
% % Save equalized symbols (eqSym) from ofdmEqualize
% outDir = 'eqSymSaved'; % Directory for saving equalized symbols
% if ~exist(outDir, 'dir'), mkdir(outDir); end
% 
% % Persistent counter for file names
% persistent eqSymSaveCount;
% if isempty(eqSymSaveCount)
%     eqSymSaveCount = 0;
% end
% eqSymSaveCount = eqSymSaveCount + 1;
% 
% % Build filename using the counter
% fname = fullfile(outDir, sprintf('%03d.mat', eqSymSaveCount));
% data = equalizedData; % Rename eqSym to 'data' for consistency in .mat file
% save(fname, 'data');
